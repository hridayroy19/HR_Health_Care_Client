...
const TabsList = ({ children }: { children: ReactNode }) => {
  return (
    <div className="flex border-b gap-11 border-gray-200 dark:border-gray-800">
      {children}
    </div>
  );
};
...
const TabsTrigger = ({ value, children }: TabsTriggerProps) => {
  ...
  return (
    <button
      ...
      className="relative px-4 py-2 text-lg font-medium rounded-t-md overflow-hidden transition-colors duration-500"
    >
      <span
        className={twMerge(
      ...
      <span
        className={twMerge(
          ...
          isActive ? "scale-y-100 bg-purple-600 dark:bg-white" : "bg-transparent"
        )}
      />
    </button>
  );
