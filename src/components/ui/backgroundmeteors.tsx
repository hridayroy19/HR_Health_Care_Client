...
interface BackgroundMeteorsProps {
  children?: ReactNode;
  className?: string; // allow extra class passing
}
...
export default function BackgroundMeteors({
  ...
  return (
    <div
      className={`relative w-full overflow-hidden bg-white dark:bg-black ${className}`}
    >
      ...
      <div
        ...
        style={{
          backgroundSize: `${gridSize}px ${gridSize}px`,
          backgroundImage:
            "linear-gradient(to right, #262626 1px, transparent 1px), linear-gradient(to bottom, #024e8b 1px, transparent 1px)",
        }}
      />
      {/* Mask overlay */}
      <div
        className="pointer-events-none absolute inset-0 flex items-center justify-center bg-purple-700 dark:bg-black
        ...
        >
          <div
            className="h-14 w-px rounded-full
              bg-gradient-to-t from-purple-800 to-transparent
